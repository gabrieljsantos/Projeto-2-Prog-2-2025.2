...
namespace mod_ADM{
    ...
    void adicionarCredito(){
        std::fstream fileCredito;
        Credito solicitor{};
        solicitor.tipo_user = NEHUMA;
        mod_ADM::openFile(fileCredito,"creditos.dat");
        char escolha;
        do{
            system("cls");
            std::cout
            <<"======= Creditos =======\n"
            <<"[1]- Listar Solicitacao\n"
            <<"[2]- Creditar Usuarios\n"
            <<"========================\n"
            <<"[X]- Voltar\n";
            std::cin>>escolha;
            ...
            case '1':{
                ...
                while(fileCredito.read((char*)&solicitor,sizeof(Credito))){
                    if(solicitor.id_opera!=0 &&solicitor.tipo_user != NEHUMA){
                            std::cout
                            <<"Cod. Sol.: "<<solicitor.id_opera<<"\n"
                            <<"Id User: "<<solicitor.id_user<<"\n"
                            <<"Usuario: "<<(solicitor.tipo_user == ALUNO ? "Aluno": "Professor")<<"\n"
                            <<"Valor Sol.: "<<solicitor.saldo<<"\n"
                            <<"Resultado: "<<(solicitor.realizado == true? "realizado":"Espera")<<"\n"
                            <<"----------------------------\n";
                    }
            ...
            case '2':{
                ...
                std::cout
                <<"==== Creditar Usuario ====\n\n"
                <<"Digite o ID da Operacao: ";
                std::cin>>buscaId;
                limparbuffer();
                fileCredito.clear();
                fileCredito.seekg(0, std::ios::end);
                int baytes = fileCredito.tellg();
                int tamanho = baytes/ sizeof(Credito);
                fileCredito.seekg(0);
                if(buscaId<=0 || buscaId > tamanho){
                    std::cout<<"Id invalido!\n";
                    break;
                }

                fileCredito.seekg((buscaId-1)*sizeof(Credito));
                fileCredito.read((char*)&solicitor,sizeof(Credito));

                if(solicitor.realizado){
                    std::cout<<"Operacao ja realizada!\n";
                    break;
                }
                if(solicitor.id_opera!=0)
                    switch (solicitor.tipo_user)
                    {
                    case ALUNO:{
                        char resposta;
                        std::fstream fileAluno;
                        mod_ADM::openFile(fileAluno,"alunos.dat");
                        Aluno aluno_ = mod_ADM::buscaAluno(fileAluno, solicitor.id_user);
                        if(aluno_.base.id!=0&&aluno_.base.ativo){
                            std::cout<<"Saldo solicitado: "<<solicitor.saldo<<"\n";
                            std::cout<<"Creditar? ";
                            std::cin>>resposta;
                            limparbuffer();
                            if(resposta=='s'|| resposta=='S'){
                                aluno_.saldo += solicitor.saldo;
                                fileAluno.seekp((aluno_.base.id-1)*sizeof(Aluno));
                                fileAluno.write((char*)&aluno_,sizeof(Aluno));
                                std::cout<<"Creditado com sucesso!\n";
                                solicitor.realizado= true;
                                fileCredito.seekp((buscaId-1)*sizeof(Credito));
                                fileCredito.write((char*)&solicitor,sizeof(Credito));
                            }
                        }
                        else std::cout<<"Voltando\n";
                    ...
                    case PROFESSOR:{
                        ...
                        Professor prof_ = mod_ADM::buscaProf(fileProf, solicitor.id_user);
                        if(prof_.base.ativo&&prof_.base.id!=0){
                            std::cout<<"Saldo solicitado: "<<solicitor.saldo<<"\n";
                            std::cout<<"Creditar? ";
                            std::cin>>resposta;
                            limparbuffer();
                            if(resposta=='s'|| resposta=='S'){
                                prof_.saldo += solicitor.saldo;
                                fileProf.seekp((prof_.base.id-1)*sizeof(Professor));
                                fileProf.write((char*)&prof_,sizeof(Professor));
                                std::cout<<"Creditado com sucesso!\n";
                                solicitor.realizado= true;
                                fileCredito.seekp((buscaId-1)*sizeof(Credito));
                                fileCredito.write((char*)&solicitor,sizeof(Credito));                        
                    ...
                    default:
                        std::cout<<"Operacao nao encontrado!\n";
                        break;
                    }
